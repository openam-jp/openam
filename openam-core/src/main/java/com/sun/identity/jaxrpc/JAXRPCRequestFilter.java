...
public class JAXRPCRequestFilter implements Filter {
     ...
     * {@code APPLICATION_SESSION}). Regular user tokens and anonymous callers are rejected.
     * The check is skipped entirely when the {@link #SKIP_AUTH_CHECK} system property is
     * {@code true}.
     *
    ...
    public static boolean isServerOrAgentAuthorized() {
        ...
        try {
            ...
            return !session.isTimedOut() && session.getType() == APPLICATION_SESSION;
        } catch (Exception e) {
            debug.warning("JAXRPCRequestFilter.isServerOrAgentAuthorized: "
