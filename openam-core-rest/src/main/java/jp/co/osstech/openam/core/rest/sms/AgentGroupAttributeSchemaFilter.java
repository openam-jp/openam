...
/**
 * AttributeSchemaFilter implementation for agent groups.
 */
public class AgentGroupAttributeSchemaFilter implements AttributeSchemaFilter {
    ...
    public boolean isTarget(AttributeSchema schema) {
        if (schema.getName().equals(AgentConfiguration.ATTR_NAME_PWD)
                || schema.getName().equals(AgentConfiguration.ATTR_CONFIG_REPO)
                || schema.getName().equals(AgentConfiguration.ATTR_NAME_GROUP)) {
            return false;
        }
