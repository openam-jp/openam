...
public class RadiusConnSecurityTest {
    ...
    /** Start a background responder that crafts a reply per the supplied {@link Responder}. */
    private void startResponder(final Responder responder) {
    ...
    public void attackerForgedAccessAccept_withZeroAuthenticator_isRejected() throws Exception {
        startResponder(new Responder() {
            @Override
            public byte[] buildResponse(byte[] req) throws Exception {
    ...
    public void responseSignedWithWrongSecret_isRejected() throws Exception {
        startResponder(new Responder() {
            @Override
            public byte[] buildResponse(byte[] req) throws Exception {
                return buildLegitimateAccept(req, "attacker-guessed-secret");
            }
        });
    ...
    public void unexpectedPacketType_isRejected() throws Exception {
        startResponder(new Responder() {
            @Override
            public byte[] buildResponse(byte[] req) throws Exception {
                return buildResponseWithCode(req, (byte) 99, SHARED_SECRET);
            }
        });
    ...
    public void healthCheckSurvivesClientSocketClose() throws Exception {
        ...
        startResponder(new Responder() {
            @Override
            public byte[] buildResponse(byte[] req) throws Exception {
                return buildResponseWithCodeAndId(req, (byte) 3, SHARED_SECRET);
            }
        });
    ...
    public void strictMode_acceptsResponseWithValidMessageAuthenticator() throws Exception {
        startResponder(new Responder() {
            @Override
            public byte[] buildResponse(byte[] req) throws Exception {
                return buildAcceptWithMessageAuthenticator(req, SHARED_SECRET);
            }
        });
